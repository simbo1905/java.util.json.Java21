...
final class EsmRenderer {
    ...
    private static void generateDefinitionFunction(StringBuilder sb, String defName, JtdNode node, RenderContext ctx) {
        ...
        // Schema paths inside a definition are absolute document positions per
        // the error-indicator semantics; instance paths continue from the caller.
        generateNodeValidation(sb, node, ctx, "v", "p", jsString("/definitions/" + jsonPointerEscape(defName)), "    ", null);
        sb.append("}\n\n");
    }

    /// Generates validation code for a node.
    ///
    /// @param discriminatorKey if non-null, this PropertiesNode is a discriminator variant
    ///                         and should skip validation of the discriminator key itself
    private static void generateNodeValidation(StringBuilder sb, JtdNode node, RenderContext ctx,
        ...
        switch (node) {
            ...
            case ElementsNode el -> {
                ...
                sb.append(indent).append("    errors.push({instancePath: ").append(pathExpr)
                  .append(", schemaPath: ").append(schemaPathExpr).append(" + ").append(jsString("/elements")).append("});\n");
            ...
            case PropertiesNode pn -> {
                ...
                sb.append(indent).append("    errors.push({instancePath: ").append(pathExpr)
                  ...
                  .append(pn.properties().isEmpty() ? jsString("/optionalProperties") : jsString("/properties")).append("});\n");
                sb.append(indent).append("} else {\n");
                ...
                for (var entry : pn.properties().entrySet()) {
                    ...
                    sb.append(indent).append("    if (!(").append(jsString(key)).append(" in ")
                      .append(valueExpr).append(")) {\n");
                    sb.append(indent).append("        errors.push({instancePath: ").append(pathExpr)
                      .append(", schemaPath: ").append(schemaPathExpr).append(" + ")
                      .append(jsString("/properties/" + jsonPointerEscape(key))).append("});\n");
                    sb.append(indent).append("    }\n");
                    
                    // Validate value if present
                    sb.append(indent).append("    if (").append(jsString(key)).append(" in ")
                      .append(valueExpr).append(") {\n");
                    final String propValue = valueExpr + "[" + jsString(key) + "]";
                    final String propPath = pathExpr + " + " + jsString("/" + jsonPointerEscape(key));
                    final String propSchemaPath = schemaPathExpr + " + " + jsString("/properties/" + jsonPointerEscape(key));
                ...
                for (var entry : pn.optionalProperties().entrySet()) {
                    ...
                    final String propValue = valueExpr + "[" + jsString(key) + "]";
                    final String propPath = pathExpr + " + " + jsString("/" + jsonPointerEscape(key));
                    final String propSchemaPath = schemaPathExpr + " + " + jsString("/optionalProperties/" + jsonPointerEscape(key));
            ...
            case ValuesNode vn -> {
                ...
                sb.append(indent).append("if (").append(valueExpr).append(" === null || typeof ")
                  .append(valueExpr).append(" !== \"object\" || Array.isArray(").append(valueExpr).append(")) {\n");
                sb.append(indent).append("    errors.push({instancePath: ").append(pathExpr)
                  .append(", schemaPath: ").append(schemaPathExpr).append(" + ").append(jsString("/values")).append("});\n");
                sb.append(indent).append("} else {\n");
            ...
            case DiscriminatorNode dn -> {
                ...
                sb.append(indent).append("if (").append(valueExpr).append(" === null || typeof ")
                  .append(valueExpr).append(" !== \"object\" || Array.isArray(").append(valueExpr).append(")) {\n");
                sb.append(indent).append("    errors.push({instancePath: ").append(pathExpr)
                  .append(", schemaPath: ").append(schemaPathExpr).append(" + ").append(jsString("/discriminator")).append("});\n");
                sb.append(indent).append("} else if (!(").append(jsString(dn.discriminator())).append(" in ")
                  .append(valueExpr).append(")) {\n");
                sb.append(indent).append("    errors.push({instancePath: ").append(pathExpr)
                  .append(", schemaPath: ").append(schemaPathExpr).append(" + ").append(jsString("/discriminator")).append("});\n");
                sb.append(indent).append("} else if (typeof ").append(valueExpr).append("[").append(jsString(dn.discriminator()))
                  .append("] !== \"string\") {\n");
                sb.append(indent).append("    errors.push({instancePath: ").append(pathExpr)
                  .append(" + ").append(jsString("/" + jsonPointerEscape(dn.discriminator())))
                  .append(", schemaPath: ").append(schemaPathExpr)
                  .append(" + \"/discriminator\"});\n");
                sb.append(indent).append("} else {\n");
                sb.append(indent).append("    const tag = ").append(valueExpr).append("[").append(jsString(dn.discriminator())).append("];\n");
                ...
                for (var entry : dn.mapping().entrySet()) {
                    ...
                    generateNodeValidation(sb, variantSchema, ctx, valueExpr, pathExpr, 
                        schemaPathExpr + " + " + jsString("/mapping/" + jsonPointerEscape(tagValue)), 
                        indent + "        ", dn.discriminator());
                }
                
                if (first) {
                    // Empty mapping: any tag value is an unknown tag
                    sb.append(indent).append("    errors.push({instancePath: ").append(pathExpr)
                      .append(" + ").append(jsString("/" + jsonPointerEscape(dn.discriminator())))
                      .append(", schemaPath: ").append(schemaPathExpr)
                      .append(" + \"/mapping\"});\n");
    ...
    private static String buildKeyCheck(String varName, Set<String> allowedKeys) {
        ...
        for (String key : allowedKeys) {
            if (!first) sb.append(" && ");
            sb.append(varName).append(" !== ").append(jsString(key));
            first = false;
        }
        return sb.toString();
    }
